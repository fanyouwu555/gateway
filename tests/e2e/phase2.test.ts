...
describe('Phase 2 End-to-End Tests', () => {
  ...
  describe('Admin Key Validation', () => {
    ...
    it('should accept admin key via query parameter (WebSocket use case)', async () => {
      const res = await app.request('/v1/ws?api_key=admin-dashboard-key-456');
      expect(res.status).toBe(200);
    });

    it('should accept non-admin key via query parameter (login verification)', async () => {
      const res = await app.request('/v1/ws?api_key=gateway-test-key-123');
      expect(res.status).toBe(200);
    });

    it('should reject invalid key via query parameter', async () => {
      const res = await app.request('/v1/ws?api_key=invalid');
      expect(res.status).toBe(401);
    });
  });
